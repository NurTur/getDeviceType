...
function generateDeviceId() {
    ...
    const deviceInfo = `${userAgent}-${screenWidth}-${screenHeight}`;
    const hashedId = btoa(deviceInfo); // Base64 encode for privacy
    return hashedId;
  }
  
  const deviceId = generateDeviceId();
  console.log('Device ID:', deviceId,'   ', window.screen.width);
